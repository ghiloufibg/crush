...
func loadSessionMessages(t *testing.T, sessionID string) []message.Message {
	...
	// A mid-iteration failure leaves Next returning false, so without
	// this the benchmark would silently run on a truncated session.
...
func BenchmarkCompact(b *testing.B) {
	...
	if err := rows.Err(); err != nil {
		b.Fatalf("Row iteration failed: %v", err)
	}
